...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function initCommandPalette() {
        ...
        // Add ⌘K button to nav if not already there
        const nav = document.querySelector('header nav');
        if (nav && !document.getElementById('cmd-k-trigger')) {
            const cmdBtn = document.createElement('button');
            cmdBtn.id = 'cmd-k-trigger';
            cmdBtn.className = 'cmd-k-btn';
            cmdBtn.title = 'Quick Menu (Cmd+K or Ctrl+K)';
            cmdBtn.innerHTML = '<kbd>⌘K</kbd> Quick Menu';
        ...
        // Palette backdrop & modal HTML
        const backdrop = document.createElement('div');
        backdrop.className = 'cmd-k-backdrop';
        backdrop.id = 'cmd-k-backdrop';
        backdrop.innerHTML = `
            <div class="cmd-k-modal" id="cmd-k-modal">
                <div class="cmd-k-header">
                    <i data-lucide="search" style="width: 18px; height: 18px; color: var(--primary-color);"></i>
                    <input type="text" class="cmd-k-input" id="cmd-k-input" placeholder="Type a command or search..." autocomplete="off" spellcheck="false">
                    <kbd style="font-family: var(--font-mono); font-size: 0.7rem; background: var(--accent-color); border: 1px solid var(--text-secondary); padding: 2px 6px; border-radius: 2px; color: var(--text-secondary);">ESC</kbd>
                </div>
                <div class="cmd-k-list" id="cmd-k-list"></div>
                <div class="cmd-k-footer">
                    <span>Navigation: <kbd style="padding: 1px 4px; border: 1px solid var(--text-secondary);">↑</kbd> <kbd style="padding: 1px 4px; border: 1px solid var(--text-secondary);">↓</kbd> to cycle</span>
                    <span>Select: <kbd style="padding: 1px 4px; border: 1px solid var(--text-secondary);">↵ Enter</kbd></span>
                </div>
            </div>
        `;
        ...
        const listContainer = document.getElementById('cmd-k-list');
        let selectedIndex = 0;

        const commands = [
            // Navigation
            { group: "Navigation", title: "Home", badge: "Page", icon: "home", action: () => window.location.href = "index.html" },
            { group: "Navigation", title: "About Jake", badge: "Bio & Globe", icon: "user", action: () => window.location.href = "about.html" },
            { group: "Navigation", title: "Projects Overview", badge: "Directory", icon: "folder", action: () => window.location.href = "projects.html" },
            { group: "Navigation", title: "Unity ML Agents Project", badge: "Simulation", icon: "cpu", action: () => window.location.href = "project-unity-ml.html" },
            { group: "Navigation", title: "AI Portfolio Development", badge: "Case Study", icon: "code", action: () => window.location.href = "project-ai-portfolio.html" },
            { group: "Navigation", title: "Resume Preview & Download", badge: "Document", icon: "file-text", action: () => window.location.href = "resume.html" },
            { group: "Navigation", title: "Experiments Laboratory", badge: "Lab", icon: "flask-conical", action: () => window.location.href = "experiments.html" },
            { group: "Navigation", title: "Photo Booth (Camera & Filters)", badge: "Interactive", icon: "camera", action: () => window.location.href = "exp-photo-booth.html" },
            { group: "Navigation", title: "Sky Hopper Game", badge: "Arcade", icon: "gamepad-2", action: () => window.location.href = "exp-circuit-hopper.html" },
            { group: "Navigation", title: "The Red Button Experiment", badge: "Psychology", icon: "alert-circle", action: () => window.location.href = "exp-red-button.html" },
            
            // Actions
            { group: "Quick Actions", title: "Toggle Dark / Light Theme", badge: "Theme", icon: "sun-moon", action: () => {
                const toggle = document.getElementById('theme-toggle');
                if (toggle) toggle.click();
                closePalette();
            }},
            { group: "Quick Actions", title: "Download Latest Resume", badge: "File", icon: "download", action: () => {
                const link = document.createElement('a');
                link.href = 'resume_latest.jpg';
                link.download = 'Jake_Altom_Resume.png';
                document.body.appendChild(link);
                link.click();
                document.body.removeChild(link);
                showPortfolioToast("Starting resume download...");
                closePalette();
            }},
            { group: "Quick Actions", title: "Copy Email Address", badge: "Contact", icon: "mail", action: () => {
                navigator.clipboard.writeText("jaltom@purdue.edu").then(() => {
                    showPortfolioToast("Copied jaltom@purdue.edu to clipboard!");
                }).catch(() => {
                    showPortfolioToast("Email: jaltom@purdue.edu");
                });
                closePalette();
            }},
            { group: "Quick Actions", title: "Open LinkedIn Profile", badge: "Social", icon: "external-link", action: () => {
                window.open("https://linkedin.com/in/jake-altom", "_blank");
                closePalette();
            }},
            { group: "Quick Actions", title: "Open GitHub Profile", badge: "Social", icon: "github", action: () => {
                window.open("https://github.com/jaltom2105", "_blank");
                closePalette();
            }},
            { group: "Quick Actions", title: "Cycle Cognitive Insight", badge: "Cognition", icon: "sparkles", action: () => {
                const insightText = document.getElementById('insight-text');
                if (insightText) {
                    insightText.scrollIntoView({ behavior: 'smooth' });
                } else {
                    showPortfolioToast("Cognitive Architecture: Neural models map heuristics to attention layers.");
                }
                closePalette();
            }}
        ];
        ...
        function renderCommands() {
            ...
            if (filteredCommands.length === 0) {
                listContainer.innerHTML = '<div class="cmd-k-empty">No matching commands found.</div>';
                return;
            }
            ...
            filteredCommands.forEach((cmd, idx) => {
                if (cmd.group !== lastGroup) {
                    ...
                    groupHeader.className = 'cmd-k-group-title';
                ...
                item.className = 'cmd-k-item' + (idx === selectedIndex ? ' selected' : '');
                item.innerHTML = `
                    <div class="cmd-k-item-left">
                        <i data-lucide="${cmd.icon}" style="width: 15px; height: 15px; opacity: 0.85;"></i>
                        <span>${cmd.title}</span>
                    </div>
                    <span class="cmd-k-item-badge">${cmd.badge}</span>
                `;
        ...
        function updateSelection() {
            const items = listContainer.querySelectorAll('.cmd-k-item');
        ...
        function ensureVisible() {
            const selected = listContainer.querySelector('.cmd-k-item.selected');
            if (selected) {
